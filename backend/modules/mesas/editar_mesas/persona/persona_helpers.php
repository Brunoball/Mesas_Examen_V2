...
function mesas_editar_persona_validar_movimiento(PDO $pdo, int $numeroOrigen, int $idPrevia, int $numeroDestino): array
{
    ...
    if (!$metaDestino) {
        return [
            'valido' => false,
            'errores' => ['No se encontró el número de mesa destino.'],
            'advertencias' => [],
        ];
    }
...
function mesas_editar_persona_obtener_destinos(PDO $pdo, int $numeroOrigen, int $idPrevia): array
{
    ...
    foreach ($destinosBase as $row) {
        ...
        if (!(bool)($validacion['valido'] ?? false)) {
            continue;
        }
...
function mesas_editar_persona_mover_previa(PDO $pdo, int $numeroOrigen, int $idPrevia, int $numeroDestino): array
{
    ...
    try {
        ...
        if ($sqlState === '23000' || str_contains($mensaje, 'Duplicate entry') || str_contains($mensaje, 'Integrity constraint violation')) {
            throw new InvalidArgumentException('No se pudo mover la previa porque ya existe una asignación equivalente en la mesa destino, o porque la cátedra/docente destino no es compatible. Probá otro número de mesa.');
        }
