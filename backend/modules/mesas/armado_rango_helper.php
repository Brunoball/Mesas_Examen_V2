...
if (!function_exists('mesas_armado_rango_obtener_tabla')) {
    function mesas_armado_rango_obtener_tabla(PDO $pdo): ?array
    {
        try {
            ...
            $stmt = $pdo->query('SELECT fecha_inicio, fecha_fin, tipo_armado FROM mesas_armado_rango_actual WHERE id = 1 LIMIT 1');
    ...
    function mesas_armado_rango_obtener_auditoria(PDO $pdo): ?array
    {
        try {
            $stmt = $pdo->query(""
                ...
                . "WHERE modulo = 'mesas'\n"
                . "  AND resultado = 1\n"
                . "  AND accion IN ('mesas_armado_crear', 'mesas_armado_crear_docentes', 'mesas_armado_docentes_crear', 'mesas_armado_crear_por_disponibilidad_docente')\n"
...
if (!function_exists('mesas_armado_rango_obtener_actual')) {
    ...
    {
        $rango = mesas_armado_rango_obtener_tabla($pdo);
        if ($rango !== null) {
            return $rango;
        }

        $rango = mesas_armado_rango_obtener_auditoria($pdo);
        if ($rango !== null) {
            return $rango;
        }
