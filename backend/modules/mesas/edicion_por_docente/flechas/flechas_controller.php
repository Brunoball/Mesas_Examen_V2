...
function mesas_editar_docentes_flechas_destinos(): void
{
    try {
        ...
        $numeroMesa = mesas_editar_docentes_flechas_int($data['numero_mesa'] ?? null, 'Debe indicar el número de mesa que desea mover.');

        json_response([
            'exito' => true,
            'data' => mesas_editar_docentes_flechas_obtener_destinos($pdo, $numeroMesa),
        ]);
        ...
        log_error($e, 'mesas_editar_docentes_flechas_destinos');
        json_response([
            'exito' => false,
...
function mesas_editar_docentes_flechas_mover(): void
{
    try {
        $pdo = db();
        mesas_armado_grupos_asegurar_tablas($pdo);
        ...
        $numeroMesa = mesas_editar_docentes_flechas_int($data['numero_mesa'] ?? null, 'Debe indicar el número de mesa que desea mover.');
        $numeroGrupoDestino = mesas_editar_docentes_flechas_int($data['numero_grupo_destino'] ?? $data['id_grupo_destino'] ?? null, 'Debe indicar el grupo destino.');

        $pdo->beginTransaction();
        $resultado = mesas_editar_docentes_flechas_mover_numero($pdo, $numeroMesa, $numeroGrupoDestino);
        ...
        if (!$resultado['movido']) {
            ...
            if (mesas_editar_docentes_flechas_numero_en_grupo($pdo, $numeroMesa, $numeroGrupoDestino)) {
                $resultado = mesas_editar_docentes_flechas_respuesta_estado_actual(
                    $pdo,
                    $numeroMesa,
                    $numeroGrupoDestino,
                    null,
        ...
        log_error($e, 'mesas_editar_docentes_flechas_mover');
        json_response([
            'exito' => false,
