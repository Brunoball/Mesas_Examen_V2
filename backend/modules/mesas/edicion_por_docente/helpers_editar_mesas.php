...
function mesas_editar_docentes_fechas_activas_armado(PDO $pdo, ?string $fechaInicio = null, ?string $fechaFin = null): array
{
    try {
...
function mesas_editar_docentes_fecha_activa_armado(PDO $pdo, string $fechaMesa, array $contexto = []): bool
{
    ...
    $fechas = mesas_editar_docentes_fechas_activas_armado($pdo, $fechaMesa, $fechaMesa);
    return !empty($fechas[$fechaMesa]);
}

function mesas_editar_docentes_tipo_permite_crear_dia_nuevo(string $tipo): bool
{
...
function mesas_editar_docentes_validar_programacion_completa(PDO $pdo, string $tipo, array $data, string $fechaMesa, int $idTurno, string $hora, array $turno, array $contexto = []): array
{
    ...
    if (!$esSlotActual
        && !mesas_editar_docentes_tipo_permite_crear_dia_nuevo($tipo)
        && !mesas_editar_docentes_fecha_activa_armado($pdo, $fechaMesa, $contexto)
    ) {
...
function mesas_editar_docentes_construir_slots_validos(PDO $pdo, string $tipo, array $data, string $fechaInicio, string $fechaFin, ?array $itemActual = null): array
{
    ...
    $fechasActivas = mesas_editar_docentes_fechas_activas_armado($pdo, $fechaInicio, $fechaFin);
    ...
    $permiteCrearDiaNuevo = mesas_editar_docentes_tipo_permite_crear_dia_nuevo($tipo);
    ...
    for ($fecha = $inicio; $fecha <= $fin; $fecha = $fecha->modify('+1 day')) {
        ...
        foreach ($turnos as $turno) {
            ...
            $esSlotActual = mesas_editar_docentes_es_slot_actual($contexto, $fechaYmd, $idTurno);
