...
function mesas_armado_fase_7_reoptimizar_no_agrupadas_core(PDO $pdo, array $opciones = []): array
{
    ...
    if ($totalPendientesInicial === 0) {
        $disponibilidadDocentesSinPendientes = mesas_armado_obtener_disponibilidad_docentes($pdo);
        $blindajeCobertura = function_exists('mesas_armado_grupos_blindar_cobertura_salida')
            ? mesas_armado_grupos_blindar_cobertura_salida($pdo, $horasTurnos, $disponibilidadDocentesSinPendientes)
            : null;
        ...
        return [
            ...
            'totales_finales' => mesas_armado_grupos_totales_finales($pdo),
    ...
    mesas_armado_reopt_reemplazar_en_grupos_llenos(
        $pdo,
        $insertGrupo,
        $grupos,
        $pendientes,
    ...
    mesas_armado_reopt_blindar_grupos_con_un_solo_docente($pdo, $grupos, $pendientes, $horasTurnos, $stats);
    ...
    if (function_exists('mesas_armado_grupos_blindar_cobertura_salida')) {
        $stats['blindaje_cobertura'] = mesas_armado_grupos_blindar_cobertura_salida($pdo, $horasTurnos, $disponibilidadDocentes);
    ...
    mesas_armado_reopt_blindar_sql_grupos_con_un_solo_docente($pdo, $horasTurnos, $stats);
    ...
    mesas_armado_reopt_limpiar_observaciones_de_agrupadas($pdo);
...
function mesas_armado_reopt_obtener_slots_disponibles(PDO $pdo, array $pendientes, array $grupos, ?string $fechaInicioRango = null, ?string $fechaFinRango = null): array
{
    ...
    $rangoExplicito = $fechaInicioRango !== null && $fechaFinRango !== null
        && function_exists('mesas_armado_fecha_valida')
        && mesas_armado_fecha_valida($fechaInicioRango)
        && mesas_armado_fecha_valida($fechaFinRango)
...
function mesas_armado_reopt_docente_compartido_permitido(array $numero, array $actual): bool
{
    if (function_exists('mesas_armado_grupos_docente_compartido_permitido')) {
        return mesas_armado_grupos_docente_compartido_permitido($numero, $actual);
    }
...
function mesas_armado_reopt_misma_area(array $numero, array $actual): bool
{
...
function mesas_armado_reopt_cantidad_docentes_distintos(array $numeros): int
{
    if (function_exists('mesas_armado_grupos_cantidad_docentes_distintos')) {
        return mesas_armado_grupos_cantidad_docentes_distintos($numeros);
    }
...
function mesas_armado_reopt_cumple_minimo_docentes_distintos(array $numeros, int $minDocentes = 2): bool
{
    if (function_exists('mesas_armado_grupos_cumple_minimo_docentes_distintos')) {
        return mesas_armado_grupos_cumple_minimo_docentes_distintos($numeros, $minDocentes);
    }
    ...
    return mesas_armado_reopt_cantidad_docentes_distintos($numeros) >= $minDocentes;
}

function mesas_armado_reopt_score_grupo_para_pendiente(array $numero, array $grupo): int
{
    $score = 0;

    foreach (($grupo['numeros'] ?? []) as $actual) {
        if (mesas_armado_reopt_docente_compartido_permitido($numero, $actual)) {
            $score += 1000000;
        } elseif (mesas_armado_reopt_misma_area($numero, $actual)) {
            $score += 10000;
        }
    }
...
function mesas_armado_reopt_ocupar_numero(array &$ocupacion, array $numero, string $fecha, int $idTurno): void
{
    foreach ($numero['docentes'] as $idDocente) {
        $idDocente = (int)$idDocente;
        if ($idDocente > 0) {
            $clave = mesas_armado_clave_ocupacion_docente($idDocente, $fecha, $idTurno);
    ...
    foreach ($numero['alumnos'] as $dni) {
        $dni = trim((string)$dni);
        if ($dni !== '') {
            $clave = mesas_armado_clave_ocupacion_alumno($dni, $fecha, $idTurno);
...
function mesas_armado_reopt_limpiar_observaciones_de_agrupadas(PDO $pdo): void
{
...
function mesas_armado_reopt_completar_grupos_existentes(
    ...
    foreach ($pendientes as $numeroMesa => $numero) {
        ...
        usort($ordenGrupos, static function ($a, $b) use ($numero, $grupos): int {
            $scoreA = mesas_armado_reopt_score_grupo_para_pendiente($numero, $grupos[$a]);
            $scoreB = mesas_armado_reopt_score_grupo_para_pendiente($numero, $grupos[$b]);
        ...
        foreach ($ordenGrupos as $numeroGrupo) {
            ...
            if (!mesas_armado_reopt_cumple_minimo_docentes_distintos(array_merge($grupo['numeros'], [$numero]))) {
                unset($grupo);
                continue;
            }
             ...
             */
            $ocupacionDisponible = mesas_armado_reopt_ocupacion_sin_grupo($ocupacion, $grupo);

            if (!mesas_armado_reopt_numero_disponible_en_slot($numero, $fecha, $idTurno, $ocupacionDisponible, $disponibilidadDocentes)) {
                unset($grupo);
                continue;
            }
...
function mesas_armado_reopt_formar_grupos_con_anclas(
    ...
    foreach ($pendientes as $numeroMesa => $ancla) {
        ...
        if (!mesas_armado_reopt_numero_disponible_en_slot($ancla, $fecha, $idTurno, $ocupacion, $disponibilidadDocentes)) {
            continue;
        }
        ...
        foreach ($pendientes as $candidatoMesa => $candidato) {
            ...
            if (!mesas_armado_reopt_numero_disponible_en_slot($candidato, $fecha, $idTurno, $ocupacion, $disponibilidadDocentes)) {
                continue;
            }
        ...
        if (count($grupoTemporal) < $minNumeros || !mesas_armado_reopt_cumple_minimo_docentes_distintos($grupoTemporal)) {
            continue;
        }
...
function mesas_armado_reopt_buscar_mejor_movimiento_con_donante(
    ...
    foreach ($grupos as $numeroGrupo => $grupo) {
        ...
        foreach (($grupo['numeros'] ?? []) as $donante) {
            ...
            if (!mesas_armado_reopt_cumple_minimo_docentes_distintos($grupoOrigenRestante)) {
                continue;
            }

            if (mesas_armado_reopt_cantidad_docentes_distintos([$pendiente, $donante]) < 2) {
                continue;
            }
            ...
            foreach ($slots as $slot) {
                ...
                if (!mesas_armado_reopt_cumple_minimo_docentes_distintos($grupoNuevo)) {
                    continue;
                }
                ...
                if (mesas_armado_reopt_docente_compartido_permitido($pendienteDestino, $donanteDestino)) {
                    $score += 500000;
                } elseif (mesas_armado_reopt_misma_area($pendienteDestino, $donanteDestino)) {
                    $score += 20000;
                }
...
function mesas_armado_reopt_ocupacion_sin_grupo(array $ocupacion, array $grupo): array
{
    ...
    foreach (($grupo['numeros'] ?? []) as $numero) {
        mesas_armado_reopt_liberar_numero($ocupacionDisponible, $numero, $fecha, $idTurno);
    }
...
function mesas_armado_reopt_reemplazar_en_grupos_llenos(
    PDO $pdo,
    PDOStatement $insertGrupo,
    array &$grupos,
    array &$pendientes,
    ...
    while (count($pendientes) > 0 && $pasada < $maxPasadas) {
        ...
        foreach ($pendientes as $numeroMesa => $pendiente) {
            if (!isset($pendientes[(int)$numeroMesa]) || !mesas_armado_reopt_es_simple_flexible($pendiente) || $pendiente['id_area'] === null) {
                continue;
            }

            $movimiento = mesas_armado_reopt_buscar_mejor_reemplazo_en_grupo_lleno(
                $pendiente,
                $grupos,
                $pendientes,
                $ocupacion,
        ...
        mesas_armado_reopt_aplicar_reemplazo_en_grupo_lleno(
            $pdo,
            $insertGrupo,
            $grupos,
            $pendientes,
...
function mesas_armado_reopt_buscar_mejor_reemplazo_en_grupo_lleno(
    array $pendiente,
    array $grupos,
    array $pendientes,
    array $ocupacion,
    ...
    foreach ($grupos as $numeroGrupo => $grupo) {
        ...
        foreach (($grupo['numeros'] ?? []) as $actual) {
            if (mesas_armado_reopt_docente_compartido_permitido($pendiente, $actual)) {
        ...
        $pendienteEnGrupo = mesas_armado_reopt_marcar_slot_en_numero($pendiente, $fechaGrupo, $turnoGrupo);
        $ocupacionSinGrupo = mesas_armado_reopt_ocupacion_sin_grupo($ocupacion, $grupo);

        if (!mesas_armado_reopt_numero_disponible_en_slot($pendienteEnGrupo, $fechaGrupo, $turnoGrupo, $ocupacionSinGrupo, $disponibilidadDocentes)) {
            continue;
        }

        foreach (($grupo['numeros'] ?? []) as $expulsado) {
            if (!mesas_armado_reopt_es_simple_flexible($expulsado)) {
                continue;
            }
            ...
            $expulsadoComparteDocente = mesas_armado_reopt_docente_compartido_permitido($pendienteEnGrupo, $expulsado);
            ...
            if (!mesas_armado_reopt_cumple_minimo_docentes_distintos($grupoReemplazado)) {
                continue;
            }
            ...
            for ($i = 0; $i < count($grupoReemplazado); $i++) {
                ...
                if (!mesas_armado_reopt_compatible_con_numeros($numero, $resto)) {
                    $compatible = false;
                    break;
                }
            }
            ...
            $ocupacionConReemplazo = mesas_armado_reopt_crear_ocupacion_desde_grupos($gruposConReemplazo);

            $destino = mesas_armado_reopt_buscar_destino_para_expulsado_de_reemplazo(
                $expulsado,
                (int)$numeroGrupo,
                $gruposConReemplazo,
                $pendientes,
...
function mesas_armado_reopt_buscar_destino_para_expulsado_de_reemplazo(
    array $expulsado,
    int $grupoOrigen,
    ...
    foreach ($gruposConReemplazo as $numeroGrupoDestino => $grupoDestino) {
        ...
        if (!mesas_armado_reopt_area_compatible($expulsado, $grupoDestino['id_area'] !== null ? (int)$grupoDestino['id_area'] : null)) {
            continue;
        }

        if (!mesas_armado_reopt_compatible_con_numeros($expulsado, $grupoDestino['numeros'] ?? [])) {
            continue;
        }

        if (!mesas_armado_reopt_cumple_minimo_docentes_distintos(array_merge($grupoDestino['numeros'] ?? [], [$expulsado]))) {
            continue;
        }

        $fecha = (string)$grupoDestino['fecha_mesa'];
        $idTurno = (int)$grupoDestino['id_turno'];
        $expulsadoDestino = mesas_armado_reopt_marcar_slot_en_numero($expulsado, $fecha, $idTurno);
        $ocupacionDisponible = mesas_armado_reopt_ocupacion_sin_grupo($ocupacionConReemplazo, $grupoDestino);

        if (!mesas_armado_reopt_numero_disponible_en_slot($expulsadoDestino, $fecha, $idTurno, $ocupacionDisponible, $disponibilidadDocentes)) {
            continue;
        }

        $score = 100000;
        foreach (($grupoDestino['numeros'] ?? []) as $actual) {
            if (mesas_armado_reopt_docente_compartido_permitido($expulsadoDestino, $actual)) {
                $score += 50000;
            } elseif (mesas_armado_reopt_misma_area($expulsadoDestino, $actual)) {
                $score += 1000;
            }
        }
    ...
    foreach ($pendientes as $numeroMesaAcompaniante => $acompaniante) {
        if ((int)$numeroMesaAcompaniante === $numeroPendienteQueEntra || !mesas_armado_reopt_es_simple_flexible($acompaniante)) {
            continue;
        }

        if (!mesas_armado_reopt_compatible_con_numeros($acompaniante, [$expulsado])) {
            continue;
        }

        if (mesas_armado_reopt_cantidad_docentes_distintos([$expulsado, $acompaniante]) < 2) {
            continue;
        }
        ...
        foreach ($slots as $slot) {
            ...
            $expulsadoDestino = mesas_armado_reopt_marcar_slot_en_numero($expulsado, $fecha, $idTurno);
            $acompanianteDestino = mesas_armado_reopt_marcar_slot_en_numero($acompaniante, $fecha, $idTurno);

            if (!mesas_armado_reopt_numero_disponible_en_slot($expulsadoDestino, $fecha, $idTurno, $ocupacionConReemplazo, $disponibilidadDocentes)) {
                continue;
            }
            if (!mesas_armado_reopt_numero_disponible_en_slot($acompanianteDestino, $fecha, $idTurno, $ocupacionConReemplazo, $disponibilidadDocentes)) {
                continue;
            }
            ...
            $score = 50000;
            if (mesas_armado_reopt_docente_compartido_permitido($expulsadoDestino, $acompanianteDestino)) {
...
function mesas_armado_reopt_aplicar_reemplazo_en_grupo_lleno(
    PDO $pdo,
    PDOStatement $insertGrupo,
    array &$grupos,
    array &$pendientes,
    ...
    mesas_armado_reopt_actualizar_slot_numero($pdo, (int)$pendienteEnGrupo['numero_mesa'], $fechaGrupo, $idTurnoGrupo);
    ...
    mesas_armado_reopt_eliminar_pendientes($pdo, [(int)$pendienteEnGrupo['numero_mesa']]);
    ...
    if (($destino['tipo'] ?? '') === 'grupo_existente') {
        ...
        mesas_armado_reopt_actualizar_slot_numero($pdo, (int)$expulsadoDestino['numero_mesa'], $fechaDestino, $idTurnoDestino);
        mesas_armado_reopt_insertar_numero_en_grupo_existente(
            $pdo,
            $insertGrupo,
        ...
        mesas_armado_reopt_actualizar_slot_numero($pdo, (int)$expulsadoDestino['numero_mesa'], $fechaDestino, $idTurnoDestino);
        mesas_armado_reopt_actualizar_slot_numero($pdo, (int)$acompanianteDestino['numero_mesa'], $fechaDestino, $idTurnoDestino);
        ...
        mesas_armado_reopt_eliminar_pendientes($pdo, [(int)$acompanianteDestino['numero_mesa']]);
...
function mesas_armado_reopt_blindar_sql_grupos_con_un_solo_docente(PDO $pdo, array $horasTurnos, array &$stats): void
{
    $numerosIndex = mesas_armado_reopt_indexar_numeros(mesas_armado_grupos_obtener_numeros_mesa($pdo));
    ...
    foreach ($invalidos as $grupo) {
        ...
        $numerosMesa = array_map('intval', mesas_armado_grupos_csv_a_array((string)($grupo['numeros_csv'] ?? '')));
        ...
        foreach ($numerosMesa as $numeroMesa) {
            ...
            mesas_armado_reopt_eliminar_pendientes($pdo, [$numeroMesa]);
            mesas_armado_grupos_insertar_no_agrupada(
                $insertNoAgrupada,
...
function mesas_armado_reopt_blindar_grupos_con_un_solo_docente(
    PDO $pdo,
    array &$grupos,
    array &$pendientes,
    array $horasTurnos,
    ...
    foreach ($grupos as $numeroGrupo => $grupo) {
        ...
        if (mesas_armado_reopt_cumple_minimo_docentes_distintos($numerosGrupo)) {
            continue;
        }
        ...
        foreach ($numerosGrupo as $numero) {
            mesas_armado_reopt_eliminar_pendientes($pdo, [(int)$numero['numero_mesa']]);
            mesas_armado_grupos_insertar_no_agrupada(
                $insertNoAgrupada,
                $numero,
        ...
        $stats['grupos_invalidos_por_un_solo_docente_blindados'][] = [
            ...
            'docentes_distintos' => mesas_armado_reopt_cantidad_docentes_distintos($numerosGrupo),
...
function mesas_armado_reopt_aplicar_movimiento_con_donante(
    ...
    if (!mesas_armado_reopt_cumple_minimo_docentes_distintos([$pendienteDestino, $donanteDestino])) {
        return;
    }
...
function mesas_armado_reopt_blindar_choques_docente_alumno_salida(
    ...
    while ($iteraciones < 80) {
        ...
        if (!mesas_armado_reopt_consolidar_un_choque_docente_en_grupos(
            $pdo,
            $grupos,
            $horasTurnos,
            $disponibilidadDocentes,
    ...
    $numerosIndex = mesas_armado_reopt_indexar_numeros(mesas_armado_grupos_obtener_numeros_mesa($pdo));
    $grupos = mesas_armado_reopt_obtener_grupos_actuales($pdo, $numerosIndex);
    mesas_armado_reopt_reubicar_no_agrupadas_con_choque(
        $pdo,
        $numerosIndex,
        $grupos,
        $horasTurnos,
...
function mesas_armado_reopt_consolidar_un_choque_docente_en_grupos(
    PDO $pdo,
    array &$grupos,
    array $horasTurnos,
    array $disponibilidadDocentes,
    ...
    foreach ($porDocenteSlot as $key => $gruposMapa) {
        ...
        for ($i = 0; $i < count($idsGrupos); $i++) {
            for ($j = $i + 1; $j < count($idsGrupos); $j++) {
                if (mesas_armado_reopt_intentar_unir_grupos_mismo_docente_slot(
                    $pdo,
                    $grupos,
                    $idsGrupos[$i],
                    $idsGrupos[$j],
        ...
        for ($i = 1; $i < count($idsGrupos); $i++) {
            $gid = $idsGrupos[$i];
            if (isset($grupos[$gid]) && mesas_armado_reopt_intentar_mover_grupo_a_otro_slot(
                $pdo,
                $grupos,
                $gid,
                $horasTurnos,
        ...
        for ($i = 1; $i < count($idsGrupos); $i++) {
            $gid = $idsGrupos[$i];
            if (isset($grupos[$gid]) && mesas_armado_reopt_enviar_grupo_a_no_agrupadas_por_choque_docente(
                $pdo,
                $grupos,
                $gid,
                $horasTurnos,
...
function mesas_armado_reopt_intentar_unir_grupos_mismo_docente_slot(
    PDO $pdo,
    array &$grupos,
    int $grupoA,
    int $grupoB,
     ...
     */
    if (!mesas_armado_reopt_numeros_forman_grupo_valido_mismo_docente_slot($numerosUnidos)) {
        return false;
    }
...
function mesas_armado_reopt_numeros_forman_grupo_valido_mismo_docente_slot(array $numeros): bool
{
    if (count($numeros) < 2) {
        return false;
    }

    if (!mesas_armado_reopt_cumple_minimo_docentes_distintos($numeros)) {
        return false;
    }
...
function mesas_armado_reopt_numeros_forman_grupo_valido(array $numeros): bool
{
    if (count($numeros) < 2) {
        return false;
    }
...
function mesas_armado_reopt_intentar_mover_grupo_a_otro_slot(
    PDO $pdo,
    array &$grupos,
    int $numeroGrupo,
    array $horasTurnos,
    ...
    foreach ($slotsDisponibles as $slot) {
        ...
        if (!mesas_armado_reopt_grupo_disponible_en_slot($pdo, $grupo, $fecha, $idTurno, $grupos, $numeroGrupo, $disponibilidadDocentes)) {
            continue;
        }

        mesas_armado_reopt_actualizar_slot_grupo_completo($pdo, $numeroGrupo, $grupo, $fecha, $idTurno, $horasTurnos);
...
function mesas_armado_reopt_grupo_disponible_en_slot(
    PDO $pdo,
    array $grupo,
    string $fecha,
    int $idTurno,
    ...
    $noAgrupadas = mesas_armado_reopt_obtener_no_agrupadas_actuales($pdo, mesas_armado_reopt_indexar_numeros(mesas_armado_grupos_obtener_numeros_mesa($pdo)));
    $ocupacion = mesas_armado_reopt_crear_ocupacion_salidas($grupos, $noAgrupadas, 'g_' . $grupoExcluido);

    foreach (($grupo['numeros'] ?? []) as $numero) {
        if (!mesas_armado_reopt_numero_disponible_en_salida($pdo, $numero, $fecha, $idTurno, $ocupacion, $disponibilidadDocentes, 'g_' . $grupoExcluido)) {
            return false;
        }
    }
...
function mesas_armado_reopt_enviar_grupo_a_no_agrupadas_por_choque_docente(
    PDO $pdo,
    array &$grupos,
    int $numeroGrupo,
    array $horasTurnos,
...
function mesas_armado_reopt_actualizar_slot_grupo_completo(PDO $pdo, int $numeroGrupo, array $grupo, string $fecha, int $idTurno, array $horasTurnos): void
{
    $hora = $horasTurnos[$idTurno] ?? null;
...
function mesas_armado_reopt_obtener_no_agrupadas_actuales(PDO $pdo, array $numerosIndex): array
{
    $stmt = $pdo->query('
...
function mesas_armado_reopt_crear_ocupacion_salidas(array $grupos, array $noAgrupadas = [], ?string $salidaExcluida = null): array
{
...
function mesas_armado_reopt_ocupacion_libre_para_salida(array $ocupacion, string $tipo, string $clave, string $salidaKey): bool
{
...
function mesas_armado_reopt_numero_disponible_en_salida(
    PDO $pdo,
    array $numero,
    string $fecha,
    int $idTurno,
    ...
    foreach (($numero['docentes'] ?? []) as $idDocente) {
        $idDocente = (int)$idDocente;
        if ($idDocente <= 0 || mesas_armado_docente_no_disponible($disponibilidadDocentes, $idDocente, $fecha, $idTurno)) {
            return false;
        }
        $clave = mesas_armado_clave_ocupacion_docente($idDocente, $fecha, $idTurno);
        if (!mesas_armado_reopt_ocupacion_libre_para_salida($ocupacion, 'docentes', $clave, $salidaKey)) {
            return false;
        }
    }
    ...
    foreach (($numero['alumnos'] ?? []) as $dni) {
        ...
        $clave = mesas_armado_clave_ocupacion_alumno($dni, $fecha, $idTurno);
        if (!mesas_armado_reopt_ocupacion_libre_para_salida($ocupacion, 'alumnos', $clave, $salidaKey)) {
            return false;
        }
    }

    return mesas_armado_reopt_slot_respeta_correlativas_numero($pdo, (int)$numero['numero_mesa'], $fecha, $idTurno);
}

function mesas_armado_reopt_reubicar_no_agrupadas_con_choque(
    PDO $pdo,
    array $numerosIndex,
    array $grupos,
    array $horasTurnos,
    ...
    $noAgrupadas = mesas_armado_reopt_obtener_no_agrupadas_actuales($pdo, $numerosIndex);
    ...
    foreach ($noAgrupadas as $idNoAgrupada => $numero) {
        ...
        $ocupacionActual = mesas_armado_reopt_crear_ocupacion_salidas($grupos, $noAgrupadas, 'n_' . (int)$idNoAgrupada);
        if (mesas_armado_reopt_numero_disponible_en_salida($pdo, $numero, $fechaActual, $turnoActual, $ocupacionActual, $disponibilidadDocentes, 'n_' . (int)$idNoAgrupada)) {
            continue;
        }

        $nuevoSlot = mesas_armado_reopt_buscar_slot_para_numero_sin_choque(
            $pdo,
            $numero,
            $slotsDisponibles,
            $grupos,
        ...
        $motivoBase = trim((string)($numero['motivo_no_agrupada'] ?? 'sin_compatibles_para_formar_grupo_de_2_a_4_en_misma_fecha_turno_area'));
        ...
        mesas_armado_reopt_actualizar_slot_numero($pdo, (int)$numero['numero_mesa'], $fechaNueva, $turnoNuevo);
...
function mesas_armado_reopt_buscar_slot_para_numero_sin_choque(
    PDO $pdo,
    array $numero,
    array $slotsDisponibles,
    array $grupos,
    ...
    foreach ($slotsDisponibles as $slot) {
        ...
        $ocupacion = mesas_armado_reopt_crear_ocupacion_salidas($grupos, $noAgrupadas, $salidaKey);
        $numeroDestino = mesas_armado_reopt_marcar_slot_en_numero($numero, $fecha, $idTurno);
        if (mesas_armado_reopt_numero_disponible_en_salida($pdo, $numeroDestino, $fecha, $idTurno, $ocupacion, $disponibilidadDocentes, $salidaKey)) {
            return [
                'fecha_mesa' => $fecha,
                'fecha' => $fecha,
                'id_turno' => $idTurno,
...
function mesas_armado_reopt_slot_respeta_correlativas_numero(PDO $pdo, int $numeroMesa, string $fechaCandidata, int $turnoCandidato): bool
{
    $stmt = $pdo->prepare("
        SELECT
    ...
    foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $row) {
        ...
        $comparacion = mesas_armado_reopt_comparar_slots($fechaCandidata, $turnoCandidato, $fechaOtra, $turnoOtro);
...
function mesas_armado_reopt_comparar_slots(string $fechaA, int $turnoA, string $fechaB, int $turnoB): int
{
