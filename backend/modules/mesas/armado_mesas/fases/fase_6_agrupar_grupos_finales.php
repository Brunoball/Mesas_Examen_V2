 ...
 * - Se valida que los docentes no esten bloqueados en el slot.
 */
...
function mesas_armado_grupos_finales_core(PDO $pdo, array $opciones = []): array
{
    ...
    try {
        ...
        foreach ($numeros as $numero) {
            $totalNumeros++;
            $motivoInvalido = mesas_armado_grupos_motivo_invalido($numero, $bloqueosDocentes);
...
function mesas_armado_grupos_motivo_invalido(array $numero, array $bloqueosDocentes): ?string
{
    ...
    foreach ($numero['docentes'] as $idDocente) {
        ...
        if (mesas_armado_docente_bloqueado($bloqueosDocentes, $idDocente, (string)$numero['fecha_mesa'], (int)$numero['id_turno'])) {
            return 'docente_bloqueado_en_fecha_turno';
        }
    }
